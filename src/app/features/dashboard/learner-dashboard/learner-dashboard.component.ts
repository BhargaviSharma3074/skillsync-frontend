...
export class LearnerDashboardComponent implements OnInit {
 ...
 private userService = inject(UserService);
  ...
  ngOnInit() {
     console.log('🎯 LearnerDashboardComponent initialized');

     console.log('🧪 Testing User API directly...');
    this.userService.getUserById(5).subscribe({
      next: (user) => console.log('✅ Direct user API test SUCCESS:', user),
      error: (err) => console.error('❌ Direct user API test FAILED:', err)
    });
